...
- Instead of `-v` option, we can use the preferred `--mount` option.
...
- Maintaining of layered architecture, creating a writeable layer, moving files across layers to enable copy on write etc. It's the **Storage Drivers**.
