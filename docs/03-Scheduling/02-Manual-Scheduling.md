...
- What do you do when you do not have a scheduler in your cluster?
  ...
  - The scheduler checks which Pod has no `nodeName` value. These Pods are candidates for scheduling.
...
- Without a scheduler, the Pods stay in a `Pending` state.
...
- The pod then gets assigned to the node. You can only specify the nodeName at creation time.

  ![sc2](../../images/sc2.PNG)

- Another way:
  - Assigning a node to an existing Pod requires to define a binding object:
  ...
  - So you must convert the YAML binding definition to JSON first.
